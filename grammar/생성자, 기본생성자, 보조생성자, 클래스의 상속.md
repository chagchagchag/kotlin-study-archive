...
kotlin은 상속 금지가 기본이다. 따라서 상속을 허용하기 위해서는 open 을 붙여줘야 한다. 아래 코드에서 `UserEvent` 클래스에는 `open` 키워드가 붙어있다는 점을 유의해서 살펴보자.
...
- kotlin 은 선언된 모든 클래스가 내부적으로는 final 클래스로 선언되는 것이 디폴트다. 만약 상속을 허용하려면 open 을 붙여서 선언해야 한다.
